...
function get_cred($connect)
{
	if(isset($_SESSION['user_id']))
	{
		$query = "
		SELECT credit_score FROM lms_user 
		WHERE lms_user.user_unique_id = '".$_SESSION['user_id']."'
		";
...
function count_ematerial($connect)
{
	$total = 0;

	$query = "SELECT COUNT(resource_id) AS Total FROM lms_ematerial";
