...
if(isset($_POST["issue_book_button"]))
{
    ...
    $query = "
        SELECT credit_score FROM lms_user 
        WHERE user_unique_id = '".trim($_POST['user_id'])."'
        ";
    ...
    foreach($statement->fetchAll() as $uuid)  {      

    if($uuid['credit_score'] < 50){
        $error .= '<li class ="text-danger">User Credit Score is insufficient to Issue Book</li>';
    }
    }
...
if(isset($_POST["book_return_button"]))
{
    if(isset($_POST["book_return_confirmation"]))
    {
        ...
        $query = "
        UPDATE lms_user
        SET lms_user.credit_score = credit_score + '".$_POST["cred-number"]."'
        WHERE lms_user.user_unique_id = '".$_SESSION["uuid"]."'
        ";
                ...
                <div class="card-body">
                    <form method="post">
                        <div class="mb-3">
                            ...
                            <input type="text" name="book_id" id="book_id" value="<?php if(isset($book_id)){echo $book_id;} ?>" class="form-control" onclick="bookapp()" />
                            <span id="book_isbn_result" onclick="bookdis()"></span>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">User Unique ID</label>
                            <input type="text" name="user_id" id="user_id" class="form-control" onclick="uidapp()" />
                            <span id="user_unique_id_result" onclick="uiddis()"></span>
                        </div>
                        <div class="mt-4 mb-0">
        ...
        else if($_GET["action"] == 'view')
        {
            ...
            if($issue_book_id > 0)
            {
                ...
                foreach($result as $row)
                {
                    ...
                    {
                        $_SESSION["uuid"] = $user_data["user_unique_id"];
                        echo '
                        <h2>User Details</h2>
                        <table class="table table-bordered">
                            <tr>
                            ...
                            <tr>
                                <th width="30%">User Credit Score</th>
                                <td width="70%">'.$user_data["credit_score"].'</td>
                            </tr>
                            <tr>
                    ...
                    if($status == "Issue")
                    {
                        ...
                        $form_item = '
                        <span class="cred">
                        <div class="cred-button" id="cred-decrease" onclick="decreaseValue()" value="Decrease Value">-5</div>
                        <input type="number" name="cred-number" id="cred-number" value="10" />
                        <div class="cred-button" id="cred-increase" onclick="increaseValue()" value="Increase Value">+5</div>
                        </span>
                        <br />
                    ...
                    if($status == 'Not Return')
                    {
                        ...
                        <span class="cred">
                        <div class="cred-button" id="cred-decrease" onclick="decreaseValue()" value="Decrease Value">-5</div>
                        <input type="number" id="cred-number" value="10" />
                        <div class="cred-button" id="cred-increase" onclick="increaseValue()" value="Increase Value">+5</div>
                        </span>
                        <br />
